...
def test_simple_linear_model_sgd():
    ...
    inp.gpu(), targ.gpu(), model.gpu()

    for _ in range(5):
        y = model(inp)
        y_torch = model_torch(inp_torch)

        # Forward pass is already wrong
        y.cpu()
...
def test_conv1d_mpool1d_model_sgd():
    ...
    model = nnn.Sequential(nnn.Conv1d(4, 32, 3, 3), nnn.MaxPool1d(2, 2), nnn.ReLU(),
                           nnn.Conv1d(32, 64, 2, 2), nnn.AvgPool1d(2, 2), nnn.Flatten(),
                           nnn.Linear(320, 1))
    optimizer = optim.SGD(model.parameters())
