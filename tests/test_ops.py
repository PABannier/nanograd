...
class TestOps(unittest.TestCase):
    ...
    def test_slice(self):
        make_test_ops([(30, 40, 20, 10)], lambda x: x[10:20, :, :, :], device=self.device)
        make_test_ops([(30, 40, 20, 10)], lambda x: x[10:20, :, :12, 5:], device=self.device)
        make_test_ops([(30, 40, 20, 10)], lambda x: x[10:20, :, :12, 0:-1], device=self.device)
    ...
    def test_pad1d(self):
        make_test_ops([(20, 3, 100)], lambda x: x.pad1d((1, 2)), lambda x: F.pad(x, (1, 2)), device=self.device)
    def test_maxpool1d(self):
        make_test_ops([(20, 3, 100)], lambda x: x.max_pool1d(3), lambda x: F.max_pool1d(x, 3), device=self.device)
    def test_avgpool1d(self):
        make_test_ops([(20, 3, 100)], lambda x: x.avg_pool1d(3), lambda x: F.avg_pool1d(x, 3), device=self.device)
    def test_conv1d(self):
        make_test_ops([(20, 3, 100), (10, 3, 3)], lambda x, w: x.conv1d(w, 1), lambda x, w: F.conv1d(x, w, stride=1), device=self.device)
        make_test_ops([(20, 3, 100), (10, 3, 3)], lambda x, w: x.conv1d(w, 3), lambda x, w: F.conv1d(x, w, stride=3), device=self.device)

    def test_pad2d(self):
        make_test_ops([(20, 3, 30, 30)], lambda x: x.pad2d((1, 2, 3, 4)), lambda x: F.pad(x, (1, 2, 3, 4)), device=self.device)
    def test_maxpool2d(self):
        make_test_ops([(20, 3, 30, 30)], lambda x: x.max_pool2d((3, 3)), lambda x: F.max_pool2d(x, kernel_size=(3, 3)), device=self.device)
    def test_avgpool2d(self):
        make_test_ops([(20, 3, 30, 30)], lambda x: x.avg_pool2d((3, 3)), lambda x: F.avg_pool2d(x, kernel_size=(3, 3)), device=self.device)
    def test_conv2d(self):
        make_test_ops([(20, 3, 30, 30), (10, 3, 3, 3)], lambda x, w: x.conv2d(w, 1), lambda x, w: F.conv2d(x, w, stride=1), device=self.device)
        make_test_ops([(20, 3, 30, 30), (10, 3, 3, 3)], lambda x, w: x.conv2d(w, 3), lambda x, w: F.conv2d(x, w, stride=3), device=self.device)
