...
def im2col(np.ndarray[DTYPE_t, ndim=4] x, int field_height, int field_width, 
           int padding, int stride):
    r"""
        Performs an im2col transform. 
        The im2col transform is a transformation to
        efficiently perform convolutions over a multi-dimensional signal.

        Args:
            x (np.ndarray): the signal to transform
            field_height (int): the kernel height
            field_width (int): the kernel width
    ...
    cdef np.ndarray[DTYPE_t, ndim=4] x_padded = np.pad(
        x, ((0, 0), (0, 0), (padding, padding), (padding, padding)), mode="constant")

    cdef np.ndarray[DTYPE_t, ndim=2] cols = np.zeros(
        (C * field_height * field_width, N * HH * WW), dtype=x.dtype)

    im2col_inner(cols, x_padded, N, C, H, W, HH, WW, field_height, field_width, padding, stride)
...
cdef int im2col_inner(np.ndarray[DTYPE_t, ndim=2] cols,
    ...
    cdef int c, yy, xx, row, ii, jj, i, col
...
def col2im_6d(np.ndarray[DTYPE_t, ndim=6] cols, int N, int C, int H, int W, 
    ...
    cdef int OH = int((H + 2 * pad - HH) / stride + 1)
    cdef int OW = int((W + 2 * pad - WW) / stride + 1)

    cdef np.ndarray[DTYPE_t, ndim=4] x_padded = np.zeros((N, C, H + 2 * pad, W + 2 * pad), dtype=cols.dtype)

    col2im_6d_inner(cols, x_padded, N, C, H, W, HH, WW, OH, OW, pad, stride)
...
cdef col2im_6d_inner(np.ndarray[DTYPE_t, ndim=6] cols,
                     np.ndarray[DTYPE_t, ndim=4] x_padded,
                     int N, int C, int H, int W, int HH, int WW,
